...
export function StatsSection() {
  return (
    <section className="relative bg-[var(--obsidian-900)] px-[clamp(1.5rem,5vw,4rem)] py-[clamp(6rem,11vw,10rem)]">
      ...
      <div className="mx-auto mt-16 grid max-w-[1100px] gap-px bg-[color:rgba(253,252,248,0.08)] md:grid-cols-4">
        {METRICS.map((m) => (
          <div
            key={m.label}
            data-cursor="hover"
          ...
          >
            <p className="font-km-mono text-[clamp(2rem,4vw,3rem)] tracking-tight text-[color:var(--ivory-50)]">
              {m.value}
              {m.suffix ? (
      ...
      <figure className="mx-auto mt-16 max-w-3xl border-l border-[color:var(--gold-primary)] px-8">
